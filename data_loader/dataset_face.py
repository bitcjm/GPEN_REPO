...
import data_loader.degradations as degradations
...
class FaceDataset(Dataset):
    def __init__(self, path, resolution=512):
        ...
        self.HQ_imgs = sorted(glob.glob(os.path.join(path, '*/*.png')))
    ...
    def __getitem__(self, index):
        ...
        try:
            img_gt = cv2.resize(img_gt, (self.resolution, self.resolution), interpolation=cv2.INTER_NEAREST)
        except:
            new_index = random.randint(0, index-1)
            self.HQ_imgs[index] = self.HQ_imgs[new_index]
            img_gt = cv2.imread(self.HQ_imgs[index], cv2.IMREAD_COLOR)
            img_gt = cv2.resize(img_gt, (self.resolution, self.resolution), interpolation=cv2.INTER_NEAREST)
