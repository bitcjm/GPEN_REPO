...
from model.modules.equalized import EqualLinear
from model.modules.fused_act import FusedLeakyReLU
from model.modules.upfirdn2d import Upfirdn2dUpsample, Upfirdn2dBlur
    ...
    def forward(
        ...
        if noise is None:
            ...
            for i in range(self.n_mlp + 1):
                ...
                noise.append(paddle.create_parameter([batch, self.channels[size], size, size],dtype='float32',attr = paddle.ParamAttr(initializer=nn.initializer.Constant(0),trainable=True)))
        ...
        if len(styles) < 2:
            ...
            latent = paddle.tile(styles[0].unsqueeze(1), repeat_times=[1, inject_index, 1])
            latent2 = paddle.tile(styles[1].unsqueeze(1), repeat_times=[1, self.n_latent - inject_index, 1])
